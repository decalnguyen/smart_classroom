...
void handleRoom() {
  ...
  if (room.length() > 0) {
    ...
    server.send(200, "text/html", "<h1>Room Saved!</h1><a href='/mqtt'>Next: Set MQTT Broker</a>");
  } else {
...
void handleMQTTPage() {
  ...
  String currentBroker = preferences.getString("broker", getMQTTBroker());
  preferences.end();

  String html = "<html><body>"
                "<h2>Set MQTT Broker</h2>"
                "<form action='/mqtt' method='POST'>"
                "MQTT Broker IP: <input type='text' name='broker' value='" + currentBroker + "'><br><br>"
...
void handleMQTT() {
  ...
  if (broker.length() > 0) {
    ...
    server.send(200, "text/html", "<h1>MQTT Broker Saved! Restarting...</h1>");
    delay(2000);
    ESP.restart();
  } else {
    server.send(400, "text/html", "<h1>Please enter MQTT Broker IP!</h1>");
  }
}
