...
void displaySensorData(const String &room, const String &ip, int light, int smoke, float temp, float humi, int led, int fan, int buzzer) {
  ...
  // Dòng 2: Light (%) + Smoke (giá trị raw)
  int lightPercent = (int)((light / 4095.0) * 100);
  display.print("L:");
...
void handleGetDisplay() {
  ...
  doc["id"] = currentRoom;
  doc["ip"] = currentIp;
  // Ánh sáng trả về số từ 0-100
  int lightPercent = (int)((currentLight / 4095.0) * 100);
  doc["light"] = lightPercent;
  // Khói trả về giá trị raw dạng số nguyên
  doc["Co2"] = currentSmoke;
