...
imgInput = face_recognition.load_image_file('Images/loi1.jpg')
imgInput = cv2.cvtColor(imgInput, cv2.COLOR_BGR2RGB)

imgTest = face_recognition.load_image_file('Images/loi2.jpg')
imgTest = cv2.cvtColor(imgTest, cv2.COLOR_BGR2RGB)

faceLoc = face_recognition.face_locations(imgInput)[0]
encodeImg = face_recognition.face_encodings(imgInput)[0]
...
result = face_recognition.compare_faces([encodeImg], encodeTest)
...
cv2.imshow('Toan Input', imgInput)
cv2.imshow('Toan Test', imgTest)
cv2.waitKey(0)
